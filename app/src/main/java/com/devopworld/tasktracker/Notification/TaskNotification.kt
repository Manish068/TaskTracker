...
//        builder.addAction(getCompleteAction(task))
    ...
    private fun buildNotification(task: TaskData) =
        NotificationCompat.Builder(context, taskNotificationChannel.getChannelId()).apply {
...
//        addAction(getSnoozeAction(task))
    ...
    private fun buildPendingIntent(task: TaskData): PendingIntent {
        ...
        Log.d(
            TAG,
            "buildPendingIntent: ${DestinationDeepLink.getTaskDetailUri(task.taskId, name!!)}"
        )
